...
export async function adminRoutes(app: FastifyInstance) {
  ...
  app.get('/api/admin/backup/export', (_req, reply) => {
    ...
    const memorySlots = db.prepare('SELECT * FROM memory_slots').all()
      ...
      if (Array.isArray(payload.documents)) {
        const insertDoc = db.prepare(`
          INSERT INTO documents (id, doc_type, doc_number, doc_date, customer_phone, customer_snapshot,
            ...
            round_off, grand_total, payment_mode, payment_status, selected_upi_id, revision_number, notes)
          VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        `)
        for (const d of payload.documents) {
          insertDoc.run(
            d.id, d.doc_type, d.doc_number, d.doc_date, d.customer_phone || null,
      ...
      if (Array.isArray(payload.documentItems)) {
        ...
        for (const item of payload.documentItems) {
          insertItem.run(
            ...
            item.gst_rate, item.cgst_rate || 0, item.cgst_amount || 0, item.sgst_rate || 0, item.sgst_amount || 0, item.total_amount
          )
        }
      }
