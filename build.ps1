...
& "$node" "$ROOT_DIR\client\node_modules\vite\bin\vite.js" build

# 2. Build Server (TypeScript)
Write-Host "`n[2/2] Building Backend (Server)..." -ForegroundColor Yellow
Set-Location "$ROOT_DIR\server"
& "$node" "$ROOT_DIR\server\node_modules\typescript\bin\tsc"

Set-Location $ROOT_DIR
Write-Host "`n✅ Build completed successfully! Assets ready in server/dist and server/public.`n" -ForegroundColor Green
